 ...
 */
export const resetmodelDataOnAllRelated = (validatable: BaseModel, modelData: IPlainObject) => {
    for (let property of Object.keys(modelData)) {
 ...
 */
export const allRelatedAreValid = (validatable: BaseModel, modelData: IPlainObject) => {
    for (let property of Object.keys(modelData)) {
        let relation = getRelationConfigForModel(validatable)[property]
        if (!relation) {
            continue
        }
