...
export function HasMany(relatedConstructorGetter: ()=>IModelConstructor, parseAliases: string[] | null = null) {
    ...
    return function(target: BaseModel, propertyName: string): any {
        ...
        let get = function(){
            //@ts-ignore
            let valueAtProperty = this.modelData[propertyName]
            if (!valueAtProperty) {
                let defaultValue = new Array<BaseModel>()
                //@ts-ignore
        ...
        let set = function(value: Array<BaseModel>) {
            //@ts-ignore
         ...
         */
        let associationsConfig: IRelationsConfig = (target.constructor as any).getRelationsConfig()
