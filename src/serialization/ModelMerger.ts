...
class ModelMerger {
    ...
    replaceWithErrorsFrom(replaceIn?: BaseModel, replaceFrom?: BaseModel) {
        ...
        for (let property of Object.keys(replaceFrom.modelData)) {
            const relation = getRelationConfigEntryForModel(replaceFrom, property)
            if (!relation) {
                continue
            }
            let replaceInValue = replaceIn.modelData[property]
            let replaceFromValue = replaceFrom.modelData[property]
    ...
    replaceErrorsForCollection(mergeToList: BaseModel[], mergeFromList: BaseModel[]) {
