...
import {IModelSerializerDoBlockParam, ModelSerializeArgs, serializationShared} from "./serializationShared"
...
export class ModelSerializer {
    ...
    serialize<T extends {}>(
        ...
        /** filters  out exclude */
        keysToIncludeInResult = serializationShared.filterOutKeysToExclude(keysToIncludeInResult, exclude)

        keysToIncludeInResult = serializationShared.filterOutDoBlockKeys(keysToIncludeInResult, doBlock)

        serializationShared.setDoBlockReturnValuesToResult(
            result, modelData, doBlock
        )
        ...
        for (let key of keysToIncludeInResult) {
            ...
            if (relationsConfig[key]) {
                result[key] = this.serializeRelationValue(
                    relationsConfig[key], rawValue, withErrors
                )
                continue
            }
    ...
    private serializeRelationValue(
        relationsConfigElement: IRelationsConfigEntry, rawValue: any, withErrors = false
    ) {
